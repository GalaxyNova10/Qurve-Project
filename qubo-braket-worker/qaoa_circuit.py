...
def run_qaoa_optimized(h: np.ndarray, J: np.ndarray, n_qubits: int,
    ...
    """Run QAOA with recursive warm-start optimization (p=1 to depth)."""
    from braket.devices import LocalSimulator
    device = LocalSimulator()
    
    # ── [QAOA_INTERFACE_AUDIT] (Fix 1) ───────────────────────────
    logger.info(
        f"[QAOA_INTERFACE_AUDIT] caller=run_qaoa_optimized depth={depth} "
    ...
    for p in range(1, depth + 1):
        ...
        if p == 1:
            ...
            # Warm start: p-layer params from (p-1)-layer + heuristic new layer
            prev_gamma = current_best_params[:p-1]
            prev_beta = current_best_params[p-1:]
            initial_guess = np.concatenate([
                prev_gamma, [prev_gamma[-1] if len(prev_gamma)>0 else np.pi/4],
                prev_beta, [prev_beta[-1] if len(prev_beta)>0 else np.pi/8]
            ])
        ...
        def objective_function(params):
            nonlocal current_best_raw_energy, current_best_params, current_best_measurements, current_best_priority
            ...
            try:
                ...
                # ── [BEST_SAMPLE_PRIORITIZATION] (Phase 6) ───────────────
                # Ranking Priority:
                # 1. Strict (Exact-K + Positive Alloc)
                # 2. Raw (Exact-K + Budget)
                # 3. Lowest Raw BQM Energy
